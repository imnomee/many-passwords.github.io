async function loadList() {
    ...
    if (result !== 'error') {
        ...
        for (var line = 1; line < lines.length; line++) {
            ...
            copyUsername.innerHTML =
                '<i class="fa fa-clone" aria-hidden="true"></i>USERNAME';
            copyUsername.setAttribute(
                'onClick',
                "copyToClipBoard('" + split(4) + '\',this,"USERNAME")'
            );
            copyUsername.setAttribute('class', 'copy-button');
            copyPassword.innerHTML =
                '<i class="fa fa-clone" aria-hidden="true"></i>PASSWORD';
            copyPassword.setAttribute(
                'onClick',
                "copyToClipBoard('" + split(5) + '\',this,"PASSWORD")'
            );
        ...
        setTimeout(() => {
            ...
            if (confirm) window.location.reload();
                ...
                loader.remove();
                output.textContent =
                    'Failed! Please update the page and try again.';
...
function copyToClipBoard(value, button, usernameorpassword) {
    ...
    setTimeout(() => {
        button.innerHTML = `<i class="fa fa-clone" aria-hidden="true"></i>${usernameorpassword}`;
    }, 3000);
